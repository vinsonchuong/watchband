...
import {Computed, State, effect} from '../signal/index.js'
...
test('making a component that takes a boolean attribute', (t) => {
  const {window} = t.context

  class Component extends BaseComponent {
    static tagName = 'wc-component'
    static attributes = ['boolean']
    boolean = new State(false)
    template = html`${new Computed(() =>
      this.boolean.get() ? html`<p>True</p>` : html`<p>False</p>`,
    )}`
  }
  ...
  component.setAttribute('boolean', '')
  t.is(component.shadowRoot.textContent, 'True')

  component.removeAttribute('boolean')
  t.is(component.shadowRoot.textContent, 'False')

  component.setAttribute('boolean', 'boolean')
  t.is(component.shadowRoot.textContent, 'True')
})

test('making a component that passes a dynamic boolean attribute', (t) => {
  const {window} = t.context

  class Parent extends BaseComponent {
    static tagName = 'wc-parent'
    boolean = new State(true)
    template = html`<${Child} boolean=${this.boolean}><//>`
  }
  ...
  t.is(childComponent.shadowRoot.textContent, 'True')

  parentComponent.component.boolean.set(false)

  t.is(childComponent.shadowRoot.textContent, 'False')
})
...
test('referencing the underlying DOM element', (t) => {
  const {window} = t.context

  class Component extends BaseComponent {
    static tagName = 'wc-component'
    static attributes = ['default-value']
    ...
    constructor(element) {
      ...
      effect(() => {
        ...
        input.value = this.defaultValue.get()
      })
    }
  }
  ...
  registerComponent(window, Component)
  window.document.body.innerHTML = `
    <wc-component default-value="Hello World!"></wc-component>
  `

  const component = window.document.querySelector('wc-component')

  t.is(component.shadowRoot.children[0].value, 'Hello World!')
})
