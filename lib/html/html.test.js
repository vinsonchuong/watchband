...
test('parsing boolean attributes', (t) => {
  ...
  {
    const {element} = html`<input type="checkbox" />`
    t.false(element.checked)
  }

  {
    const {element} = html`<input type="checkbox" checked=${false} />`
    t.false(element.checked)
  }
})
...
test('setting properties via prop: attribute', (t) => {
  ...
  const html = makeHtml(window)
  const {element} = html`<input prop:value="Hello World!" />`
...
test('using DOM Parts for properties', (t) => {
  ...
  const html = makeHtml(window)
  const {element, parts} = html`<input prop:value="{value}" />`
...
test('binding signals to element properties', (t) => {
  ...
  const {element} = html`<input prop:value=${value} />`
...
test('binding a list signal to HTML', (t) => {
  ...
  list.set([html`<li>Something</li>`, html`<li>Else</li>`])

  t.is(element.children.length, 2)
  t.is(element.children[0].textContent, 'Something')
  t.is(element.children[1].textContent, 'Else')
